...
function Home() {
  ...
  return (
    <>
      ...
      <Hero />
      <section className="reasons-section position-relative bg-dark">
        {" "}
        {/* py-5 for vertical padding, bg-dark for background */}
        <h2 className="reasons-heading  position-absolute m-3 top-0 start-0 z-2 text-light">
          Trending Films
        </h2>{" "}
        {/* text-start for left align, mb-5 for margin-bottom, text-light for heading color */}
        <Trending images={movies} />
      </section>
      <section className="reasons-section py-5 bg-dark">
        {" "}
        {/* py-5 for vertical padding, bg-dark for background */}
        <Container>
          {" "}
          {/* Bootstrap Container for responsive width */}
          <h2 className="reasons-heading text-start mb-5 text-light">
            More reasons to join
          </h2>{" "}
          {/* text-start for left align, mb-5 for margin-bottom, text-light for heading color */}
          <Row xs={1} md={2} lg={4} className="g-4">
            {" "}
            {/* Responsive grid with gutters */}
            {reasonsData.map((reason, index) => (
              <Col key={index} className="d-flex">
                {" "}
                {/* d-flex to make columns flex containers for consistent card height */}
                <ReasonCard
                  icon={reason.icon}
                  title={reason.title}
